...
public class Maze {
	...
	private boolean solve(Position p) {
		...
		if (p.equals(goal)) {
			...
			if(mazeMatrix[p.row][p.column-1].equals(Cell.OPEN)) {
			...
			if(mazeMatrix[p.row - 1][p.column].equals(Cell.OPEN)){
		...
		// Om vi är här, innebär det att vi inte kan gå vidare från denna position
		return false;
	}
